...
const EditService = () => {
  ...
  const { data: existingServices, refetch } = useGetServicesQuery();
  ...
  useEffect(() => {
    if (service) {
      setFormData({
        ...
        price: service.price || "",
        estimatedDuration: service.estimatedDuration || { days: 0, hours: 0 },
        materials: service.materials || [],
        laborCost: service.laborCost || "",
        applicableShoeTypes: service.applicableShoeTypes || [],
        note: service.note || "",
      });
      setExistingImages(service.images || []);
      setCalculatedProfit(service.profit || 0);
    }
  }, [service]);
  ...
  const handleSubmit = async (e) => {
    ...
    existingImages.forEach((img) => {
      formDataToSend.append("image", img.link);
    });

    try {
      await editService({ id: service._id, body: formDataToSend }).unwrap();
      ...
      iziToast.error({
        title: "Error",
        message:
          err?.data?.error || "An error occurred while updating the service",
        position: "topRight",
        timeout: 3000,
      });
    }
